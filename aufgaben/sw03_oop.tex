...
\lstinputlisting{../workspace/formaladdress/src/formaladdress/FormalAddress.java}
...
setPrice has a return type whereas a constructor has no return type. 
...
this? }\\
increace() is a mutator. 
\begin{lstlisting}[caption=Code to show that increase() changes score]
...
within BlueJ? }\\
All ticket machine objects have the same value within the price field. 
...
When amount is 0 there is no error message printed. 
...
different values? }\\
The field visible was true when the circle was visible and false when the 
circle was false. 
...
\subsection{Learnteam-Questions OOP3}
\subsubsection*{Exercise 1}
\verb?2.5 * 2 + 3 = 8.0?\\
\verb?2.5 * (2 + 3) = 12.5?\\
\verb?(int) 2.5 * 2 + 3 = 8?\\
...
\verb?(int) (2.5 * 2 + 3) = 8?\\
\verb?(int) (2.5 * (2 + 3)) = 12?\\
\verb?(int) 2.5 * 2 + (float) 3 = 8.0?\\

\subsubsection*{Exercise 2}
A convention is an agreement how to write accessor methods. \\
Such a convention makes the code readable for other programmers. \\
\begin{lstlisting}
getVorname()
setVorname(String newname)

getStunden()
setStunden(float newhour)

getPersonalNumber()
setPersonalNumber(int newnumber)

getMeinobjekt()
setMeinobjekt(Objekt newObjekt)
\end{lstlisting}
